...
const INITIAL_DISTRICT_TASKS = [];

export function DistrictTasks() {
  const { user } = useAuth();
  const { isDark } = useTheme();
  const [tasks, setTasks] = useState(INITIAL_DISTRICT_TASKS);
  ...
  const filteredTasks = statusFilter === 'All'
    ? tasks
    : tasks.filter(t => t.status.toLowerCase() === statusFilter.toLowerCase());
  ...
  const priorityColor = (priority) => {
    switch (priority) {
      case 'Urgent':
        return 'bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-950/60 dark:text-rose-400 dark:border-rose-900/50';
      case 'High':
        return 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/60 dark:text-amber-400 dark:border-amber-900/50';
      case 'Medium':
        return 'bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-950/60 dark:text-blue-400 dark:border-blue-900/50';
      default:
          ...
          >
            <CheckCircle2 className={`w-4 h-4 ${
              row.status === 'Completed'
                ? 'text-emerald-500 fill-emerald-100 dark:fill-emerald-950/60'
                : 'text-slate-400'
            }`} />
          </button>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
          Operational action queue, compliance deliverables, and territorial workflow tracking across {district} District.
        </p>
      </div>
      ...
      {/* Main Table */}
      <DataTable
        title="Active District Administrative Tasks"
        ...
        exportFileName="district_tasks.csv"
      />
    </div>
  );
}
