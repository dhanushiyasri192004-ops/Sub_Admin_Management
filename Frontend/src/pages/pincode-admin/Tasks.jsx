...
export function PincodeTasks() {
  ...
  const [tasks] = useState(INITIAL_PINCODE_TASKS);
    ...
    <div className="space-y-4">
      ...
      <div>
        <h2 className="text-lg font-bold text-slate-900 dark:text-white">
          Pincode Administrative Tasks
        </h2>
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
          Local operational action queue, merchant deliverables, and field workflow tracking for PIN: {pincode}.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="admin-card p-3 bg-white dark:bg-[#131f37] border border-slate-200 dark:border-[#1f3358] rounded-xl shadow-xs">
          ...
          <div className="text-xl font-bold text-slate-900 dark:text-white mt-1.5">{inProgressCount}</div>
        </div>
        ...
        <div className="admin-card p-3 bg-white dark:bg-[#131f37] border border-slate-200 dark:border-[#1f3358] rounded-xl shadow-xs">
          ...
          <div className="text-xl font-bold text-slate-900 dark:text-white mt-1.5">{completedCount}</div>
        </div>
      </div>

      {/* Main Table */}
      <DataTable
        title="Active Pincode Administrative Tasks"
        subtitle="Local action queue and compliance deliverables"
        columns={columns}
        data={filteredTasks}
        loading={loading}
        ...
        exportFileName="pincode_tasks.csv"
      />
    </div>
  );
}
