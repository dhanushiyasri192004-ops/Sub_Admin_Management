    ...
    <div className="space-y-6">
      ...
      <div>
        <h2 className="text-xl font-bold text-slate-900 dark:text-white">
          State Queries & Grievance Desk
        </h2>
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
          Review, investigate, and resolve field queries, merchant disputes, and territorial escalations across all state jurisdictions.
        </p>
      </div>
      ...
      {/* Main Table */}
      <DataTable
        title="Jurisdictional Queries Ledger"
        subtitle="Track inquiry status, SLA compliance, and stakeholder escalations"
        columns={columns}
        data={filteredQueries}
        loading={loading}
        onRefresh={handleRefresh}
        searchPlaceholder="Search query subject, ID, or submitter..."
        filterOptions={filterOptions}
        activeFilter={statusFilter}
        onFilterChange={setStatusFilter}
        exportFileName="state_queries.csv"
      />
    </div>
  );
}
